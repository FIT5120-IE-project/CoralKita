<template>
  <div class="ai-classification-container">
    ...
    <div class="back-button-container">
      <button class="back-button" @click="goToAITools">
        <span class="back-icon">&lt;</span>
        <span class="back-text">{{ $t('aiTools.common.backToTools') }}</span>
      </button>
    </div>
          ...
          <div class="result-card">
            <div class="result-header">
              ...
              <div class="result-info">
                <h4>{{ getPredictionDisplayText(getPredictionClass()) }}</h4>
                <p>{{ $t('aiTools.results.species') }}</p>
                <div v-if="getPredictionData().probabilities" class="probabilities-display">
                  <div class="probabilities-title">{{ $t('aiTools.results.probabilities') }}:</div>
                  <div class="probability-item" v-for="(prob, class_name) in getPredictionData().probabilities" :key="class_name">
                    <span class="class-name">{{ getPredictionDisplayText(class_name) }}:</span>
                    <span class="probability-value">{{ (getNormalizedProbability(prob) * 100).toFixed(1) }}%</span>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async classifyImage() {
      ...
      try {
        ...
        if (result.code === 1 && result.data) {
          // Save complete prediction result data
     ...
     */
    getPredictionDisplayText(predictedClass) {
      if (predictedClass === 'healthy_corals') {
        return this.$t('aiTools.results.healthy')
      } else if (predictedClass === 'bleached_corals') {
        return this.$t('aiTools.results.bleached')
      }
     ...
     */
    getPredictionData() {
      if (typeof this.classificationResult === 'string') {
        return { predicted_class: this.classificationResult }
      }
     ...
     */
    getPredictionClass() {
      const data = this.getPredictionData()
      return data.predicted_class || this.classificationResult
    },

    /**
     * Convert raw probability to 0-1 range probability
     */
    getNormalizedProbability(rawProb) {
      // Use softmax function to convert raw logits to probability
      if (typeof rawProb !== 'number') return 0
      
      // Get all probability values
      const probabilities = this.getPredictionData().probabilities
      if (!probabilities) return 0
      
      const values = Object.values(probabilities).map(v => typeof v === 'number' ? v : 0)
      ...
      const expRaw = Math.exp(rawProb - maxVal)
      return expRaw / sumExp
    },
    ...
    async loadBackgroundImage() {
      try {
        ...
        console.log('AI Classification background loaded (WebP):', this.backgroundImageUrl)
      } catch (webpError) {
        console.warn('WebP loading failed, trying JPG:', webpError)
        try {
          this.backgroundImageUrl = await ossService.getAIToolsBackgroundUrl()
          document.documentElement.style.setProperty('--bg-image', `url(${this.backgroundImageUrl})`)
          console.log('AI Classification background loaded (JPG):', this.backgroundImageUrl)
        } catch (jpgError) {
...
/* Navigation bar styles - consistent with Education page */
.top-nav {
...
.back-button:hover {
  ...
  box-shadow: 
    0 12px 35px rgba(0, 119, 190, 0.4),
